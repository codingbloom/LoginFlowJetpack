...
private class ComposableBackNavigationHandler(enabled: Boolean) : OnBackPressedCallback(enabled = enabled) {
...
internal fun ComposableHandler(
    ...
    val handler = remember{ComposableBackNavigationHandler(enabled = enabled)}
...
internal fun SystemBackButtonHandler(onBackPressed: () -> Unit) {
    ...
    CompositionLocalProvider(
        ...
        ComposableHandler {onBackPressed()}
    }
}